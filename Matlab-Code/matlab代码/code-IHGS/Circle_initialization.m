...
function Positions=Circle_initialization(SearchAgents_no,dim,ub,lb)
...
if Boundary_no==1
    ...
    for i=1:dim
        for g=2:SearchAgents_no
            Positions(g,i)=mod(Positions(g-1,i)+0.2-0.25.*pi.*sin(2.*pi.*Positions(g-1,i)),1).*(ub-lb)+lb;%Circle
        end
    end
end
...
if Boundary_no>1
    for i=1:dim
        ...
        for g=2:SearchAgents_no
            Positions(g,i)=mod(Positions(g-1,i)+0.2-0.25.*pi.*sin(2.*pi.*Positions(g-1,i)),1).*(ub_i-lb_i)+lb_i;%Circle
        end
    end
end