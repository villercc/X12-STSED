...
max_iter=20;%最大迭代次数
N=100;%种群数量
ArchiveMaxSize=200;%外部档案存储大小

Archive_X=zeros(100,dim);
Archive_F=ones(100,obj_no)*inf;

Archive_member_no=0;%外部档案中非劣解个数

r=(ub-lb)/2;%邻域半径
V_max=(ub(1)-lb(1))/10;%最大速度

Food_fitness=inf*ones(1,obj_no);%食物的适应度及位置
Food_pos=zeros(dim,1);

Enemy_fitness=-inf*ones(1,obj_no);%天敌的适应度及位置
Enemy_pos=zeros(dim,1);

X=initialization(N,dim,ub,lb);%初始化蜻蜓种群的位置X
fitness=zeros(N,2);%初始化适应度值

DeltaX=initialization(N,dim,ub,lb);%初始化蜻蜓种群的步长向量
iter=0;%迭代次数

position_history=zeros(N,max_iter,dim);%位置历史

for iter=1:max_iter
    
    r=(ub-lb)/4+((ub-lb)*(iter/max_iter)*2);%更新邻域半径
    
    w=0.9-iter*((0.9-0.2)/max_iter);%更新惯性权重
    ...
    if iter<(3*max_iter/4)
        s=my_c;             % Seperation weight分离权重
        a=my_c;             % Alignment weight对齐权重
        c=my_c;             % Cohesion weight凝聚权重
        f=2*rand;           % Food attraction weight猎物吸引力权重
        e=my_c;             % Enemy distraction weight避敌权重
    else
