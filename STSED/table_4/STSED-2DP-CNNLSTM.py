...
if __name__ == '__main__':
    model = Sequential()
    model.add(
        TimeDistributed(Conv1D(filters=36, kernel_size=3, activation='relu', input_shape=(None, 1, testX.shape[1]))))
    ...
    # trainPre_BDX = trainPre.reshape(-1)
    # trainPre_BDX=pd.DataFrame(trainPre_BDX)
    # trainPre_BDX.to_csv('C:/Users/Administrator/Desktop/XI-2/data/pre_result/cnnbdx_train.csv', header=False)

    testPre = testPredict * ((np.max(dataset) - np.min(dataset))) + np.min(dataset)
    testPre_BDX = testPre.reshape(-1)
    # testPre_BDX=pd.DataFrame(testPre_BDX)
    # testPre_BDX.to_csv('C:/Users/Administrator/Desktop/XI-2/data/pre_result/cnnbdx_test.csv', header=False)
